...
Snapshots the latest output of run_minimal_pipeline.py into a fresh review_round_N/
...
Run run_minimal_pipeline.py first; this script does not run the pipeline.

Usage:
    python start_review_round.py --archive crcns
    python start_review_round.py --archive crcns --samples-per-class 50 --include-neither false
"""
...
from build_minimal_review import build_review_html
...
def _snapshot_pipeline_files(archive_dir: Path, review_round_dir: Path) -> Path:
    ...
    for filename in ("datasets.json", "citation_contexts.json", "classifications.json"):
        ...
        if not source.exists():
            print(
                f"Error: {source} not found. Run run_minimal_pipeline.py --archive "
...
def main():
    ...
    archive_dir = Path("output/minimal") / args.archive
