...
Batch mode runs the same worklist through OpenAI's Batch API at half the
interactive rate, in exchange for results arriving within a 24-hour window.
It needs OPENAI_API_KEY in .env and writes into the same per-pair cache, so
batch and interactive runs can be mixed freely:
