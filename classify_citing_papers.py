...
6. Reading a numerical value, fitted parameter, equation constant, summary statistic, or figure data point from the text or figures of the primary paper — for any purpose, including parameterizing a model or simulation — is NOT data reuse. The distinction is whether the citing paper downloaded the underlying data files or recordings. If the only quantitative borrowing is values reported in the primary paper itself (e.g., "we used parameters alpha and A from [ref]", "we adopted the time constant reported by [ref]"), classify as MENTION. Downloading the open data and using it as input to a simulation or model IS data reuse; extracting numbers reported in the primary paper to parameterize a model is NOT.
...
12. Review articles, perspectives, commentaries, opinion pieces, and editorials are MENTION, not REUSE — even when authored by the dataset's primary author, and even when they describe the dataset in detail. Review-style articles discuss data; they do not reanalyze it. Signals that the citing paper is a review/perspective: titles or section headings containing "Review", "Perspective", "Commentary", "Opinion"; the article surveys multiple prior studies rather than presenting new analyses on raw recordings; phrasing like "in this review we discuss", "here we summarize", or "as we have previously shown". When such signals are present, classify as MENTION even if shared authorship or extensive discussion of the dataset suggests otherwise.
