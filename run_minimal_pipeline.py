...
def stage1_datasets(adapter, limit):
    data = adapter.build_datasets_json()
    if limit is not None:
        data["results"] = sorted(data["results"], key=lambda r: r["dataset_id"])[:limit]
        data["count"] = len(data["results"])
        (adapter.output_dir / "datasets.json").write_text(json.dumps(data, indent=2))
    print(f"  {data['count']} datasets selected", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--limit", type=int, default=None,
                        help="Cap to first N datasets (sorted by ID). Default: all.")
    ...
    stage1_datasets(adapter, args.limit)
