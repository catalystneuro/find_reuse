...
Here we address this gap with the first comprehensive, automated, LLM-assisted pipeline for tracking data reuse across an entire neurophysiology repository. The pipeline reaches the literature by two routes. The direct route finds papers that name a dandiset by its identifier or DOI. The indirect route starts from the publication that first described each dataset enumerates papers citing it, then determines whether each citing paper is reusing the data or merely referencing the paper. The LLM classifier also reports which modality of the dataset was reused and which archive, if any, the authors name as the source, which distinguishes reuse of data obtained from DANDI from reuse of the same experiments obtained through another channel. Methods are given in full in Section 2.

Applied to the complete DANDI Archive, this pipeline identifies 1,065 reuse events across 796 papers, and reuse is growing. Different-laboratory reuse constitutes the majority of reuse events, and the mean cumulative number of reuse papers per dandiset follows a Richards growth curve that peaks in rate roughly four years after public data release. Tool and method demonstration is the most common form of reuse, in which a new computational method is validated against an established public dataset, followed by novel analyses that put new scientific questions to data already collected. Finally, we document a systematic failure of conventional citation metrics: 93% of reuse papers do not cite the dataset record itself, meaning that DOI-based impact tracking, as currently implemented by DataCite and recommended by FORCE11 data citation principles, would miss the vast majority of scientifically meaningful reuse. These findings have direct implications for how funding agencies should evaluate the impact of data-sharing mandates and how repositories should design attribution infrastructure.
...
All candidate DOIs, whether extracted from metadata or proposed by the LLM, were validated by resolving them against CrossRef and OpenAlex. DOIs that failed resolution were subjected to a title-based recovery procedure that searched sequentially through Europe PMC (full-text query), OpenAlex (title filter), and CrossRef (title query with majority-word-overlap matching). Of the LLM-proposed DOIs, 175 were validated successfully and 106 were rejected as hallucinations. After merging all sources, 359 of the 554 non-empty dandisets (65%) were linked to 346 unique primary papers.
...
A post-classification filter was applied to Patch-seq datasets (11 dandisets containing multimodal electrophysiology, morphology, and transcriptomics data). DANDI hosts the electrophysiology and morphology components of these datasets, while the transcriptomic data is typically distributed through other platforms (GEO, CELLxGENE, NeMO Archive). Each Patch-seq REUSE entry was sent to Gemini 3 Flash with a prompt asking which data modality the paper used. Entries classified as "transcriptomics_only" (169 of 266 Patch-seq entries) were reclassified as MENTION, as the data they used is not hosted on DANDI. Preprint/published duplicate pairs were also removed by matching on normalized titles, preferring the journal version when both existed.
...
Our pipeline identified 1,065 paper-dandiset reuse events from 796 unique citing papers across the DANDI Archive, after deduplicating preprint/published pairs and excluding papers that reused only the transcriptomic component of Patch-seq datasets (which is not hosted on DANDI; see Methods). Of the 796 unique reuse papers, 729 (92%) were discovered exclusively through the citation pipeline, 25 (3%) exclusively through direct dandiset references (DANDI DOIs or URLs in the paper text), and 42 (5%) by both methods. These events span the full diversity of the archive: 727 (68%) were classified as different-lab reuse, in which the citing authors have no institutional or authorship overlap with the depositing team, while the remaining 324 (30%) represent same-lab reuse, in which the original data creators published subsequent analyses of their own deposited datasets.
...
An Andersen-Gill Cox proportional hazards model identified several dandiset-level features significantly associated with rates of different-lab reuse (Figure 5). Primary-paper citation count was the strongest predictor (HR = 4.1 per 10-fold increase in citations, p < 0.001), confirming that the visibility of the associated publication is the key driver of downstream data reuse. Allen Institute provenance was the second-strongest predictor (HR = 3.4, p < 0.001), indicating that Allen Institute dandisets attract reuse at approximately 3.4 times the rate of comparable non-Allen datasets after controlling for all other covariates. Human data (HR = 2.5, p < 0.001), benchmark datasets (HR = 2.0, p < 0.001), and mouse data (HR = 1.8, p < 0.001) were all associated with significantly higher reuse rates. Journal impact factor (HR = 1.4 per 10-fold increase in h-index, p = 0.04), electrophysiology modality (HR = 1.3, p = 0.03), and dataset size (HR = 1.2 per 10-fold increase in GB, p < 0.001) were also significant positive predictors. Non-human primate data was not a significant predictor (HR = 1.3, p = 0.28) after controlling for other covariates, and number of subjects was not significant (p = 0.46).

Calcium imaging modality was associated with significantly lower reuse rates (HR = 0.5, p < 0.001), likely reflecting the younger age of imaging datasets on DANDI and the higher computational barriers to reusing calcium imaging data compared to spike-sorted electrophysiology. The model achieved a concordance index of 0.61.
...
We identified 1,065 reuse events involving 796 unique papers across the DANDI Archive, a substantial body of secondary data use that has grown continuously since the repository's founding. Of these reuse events, 68% originated from groups with no authorship overlap with the original data depositors, confirming that the observed activity constitutes genuine external impact rather than self-citation or intra-lab replication.
...
The Andersen-Gill regression analysis provides actionable insight into what drives different-lab reuse. The most actionable finding is the outsized role of primary-paper citation count (HR = 4.1 per 10-fold increase): a dataset associated with a widely cited paper attracts reuse at over four times the rate of one associated with a less-cited paper, controlling for species, modality, and dataset size. Allen Institute provenance remains a strong predictor (HR = 3.4), and journal impact factor independently contributes (HR = 1.4), suggesting that both the venue and the reception of the primary paper matter.

This finding has practical implications for repositories and funders. Repositories seeking to maximize reuse should prioritize onboarding data from high-impact research programs whose publications will naturally attract attention to the associated datasets. Funders evaluating the impact of data sharing mandates should recognize that the primary lever for increasing reuse is not the size of datasets but the scientific prominence of the associated research. Notably, after filtering out papers that reused only the transcriptomic component of Patch-seq datasets (which is not hosted on DANDI), benchmark datasets emerged as a significant predictor of reuse (HR = 2.1), suggesting that formal benchmark designations do independently drive reuse of electrophysiology data.

The substantial species effects are also noteworthy. Human datasets are reused at significantly higher rates than other species (HR = 2.5), even after controlling for citation count and other covariates. Human intracranial recordings require neurosurgical access and are among the most expensive data to collect, suggesting that data sharing mandates deliver disproportionate return on investment for expensive-to-collect data, precisely the regime where avoiding redundant data collection matters most. Non-human primate data was not a significant independent predictor (HR = 1.3, p = 0.28) after filtering out transcriptomics-only Patch-seq reuse and controlling for other covariates.

The negative association of calcium imaging modality with reuse (HR = 0.5) warrants attention. This may reflect both the relative youth of DANDI's imaging collections and genuine barriers to reusing calcium imaging data, including large file sizes, heterogeneous preprocessing pipelines, and the lack of standardized spike-sorting equivalents for imaging data. As the imaging community develops more mature analysis standards and as imaging datasets on DANDI age, this gap may narrow.

### 4.6 The Dataset Citation Gap

One of the most practically significant findings is the near-complete absence of direct dataset citations in secondary papers. Across our corpus, 93% of reuse papers cited only the associated primary publication, while just 7% included a direct citation to the dandiset DOI or URL. This gap fundamentally undermines the discoverability of reuse and the ability of data depositors to receive appropriate credit.
