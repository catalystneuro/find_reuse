...
For each primary paper identified in Section 2.1, we used the OpenAlex API to retrieve the full list of papers that cited it. Citing papers were filtered to those with a publication date on or after the dandiset's creation date, the assumption being that any reuse necessarily postdates data availability. Each citing paper was resolved to its OpenAlex work ID via a DOI lookup, and paginated queries against the `cites:{work_id}` filter retrieved up to 200 records per page, collecting the DOI, title, publication date, and journal for each record.
